...
/// </summary>
[Authorize]
public class AccessoryTypeHub : Hub
{
    ...
    public AccessoryTypeHub(ISimpleGenericModelService<AccessoryType> service)
    {
        _service = service;
    }
    ...
    /// </summary>
    /// <returns>Список изменяемых записей в виде List</returns>
    [Authorize]
    ...
    {
        BaseTitleModel? accessoryType = await _service.GetFirstAsync(addedId);
        if (_service.HasErrors || accessoryType == null)
            return;
        await Clients.Others.SendAsync("Added", accessoryType.Id, accessoryType.Title);
    }
    ...
    {
        BaseTitleModel? accessoryType = await _service.GetFirstAsync(changedId);
        if (_service.HasErrors || accessoryType == null)
            return;
        int id = _editMapping.GetEdit(_connections.GetConnections(Context.ConnectionId));
        _editMapping.Remove(_connections.GetConnections(Context.ConnectionId));
        await OpenEdit(id);
        await Clients.Others.SendAsync("Changed", accessoryType.Id, accessoryType.Title);
    }
