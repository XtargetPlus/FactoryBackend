...
public class MaterialHub : Hub
{
    ...
    public MaterialHub(ISimpleGenericModelService<Material> service)
    {
        _service = service;
    }
    ...
    {
        BaseTitleModel? material = await _service.GetFirstAsync(addedId);
        if (_service.HasErrors || material == null)
            return;
        await Clients.Others.SendAsync("Added", material.Id, material.Title);
    }
    ...
    {
        BaseTitleModel? material = await _service.GetFirstAsync(changedId);
        if (_service.HasErrors || material == null)
            return;
        int id = _editMapping.GetEdit(_connections.GetConnections(Context.ConnectionId));
        _editMapping.Remove(_connections.GetConnections(Context.ConnectionId));
        await OpenEdit(id);
        await Clients.Others.SendAsync("Changed", material.Id, material.Title);
    }
