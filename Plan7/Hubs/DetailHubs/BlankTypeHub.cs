...
public class BlankTypeHub : Hub
{
    ...
    public BlankTypeHub(ISimpleGenericModelService<BlankType> service)
    {
        _service = service;
    }
    ...
    {
        BaseTitleModel? blankType = await _service.GetFirstAsync(addedId);
        if (_service.HasErrors || blankType == null)
            return;
        await Clients.Others.SendAsync("Added", blankType.Id, blankType.Title);
    }
    ...
    {
        BaseTitleModel? blankType = await _service.GetFirstAsync(changedId);
        if (_service.HasErrors || blankType == null)
            return;
        int id = _editMapping.GetEdit(Context.ConnectionId);
        _editMapping.Remove(Context.ConnectionId);
        await OpenEdit(id);
        await Clients.Others.SendAsync("Changed", blankType.Id, blankType.Title);
    }
