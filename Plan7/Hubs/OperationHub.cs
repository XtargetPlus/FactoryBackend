...
public class OperationHub : Hub
{
    ...
    public OperationHub(IOperationService operationRepository, IOperationCountService count)
    {
    ...
    {
        OperationDto? outOrg = await _operationRepository.GetFirstAsync(addedId);
        if (_operationRepository.HasErrors || outOrg == null)
            return;
        await Clients.Others.SendAsync("Added", outOrg.Id, outOrg.ShortName, outOrg.FullName, await _count.GetAllAsync());
    }
    ...
    {
        OperationDto? outOrg = await _operationRepository.GetFirstAsync(changedId);
        if (_operationRepository.HasErrors || outOrg == null)
            return;
        int id = _editMapping.GetEdit(_connections.GetConnections(Context.ConnectionId));
        _editMapping.Remove(_connections.GetConnections(Context.ConnectionId));
        await OpenEdit(id);
        await Clients.Others.SendAsync("Changed", outOrg.Id, outOrg.ShortName, outOrg.FullName);
    }
