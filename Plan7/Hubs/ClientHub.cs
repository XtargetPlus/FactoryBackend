...
public class ClientHub : Hub
{
    ...
    public ClientHub(IClientService clientRepository, IClientCountService count)
    {
    ...
    {
        BaseDto? client = await _clientRepository.GetFirstAsync(addedId);
        if (_clientRepository.HasErrors || client == null)
            return;
        await Clients.Others.SendAsync("Added", client.Id, client.Title, await _count.GetAllAsync());
    }
    ...
    {
        BaseDto? client = await _clientRepository.GetFirstAsync(changedId);
        if (_clientRepository.HasErrors || client == null)
            return;
        int id = _editMapping.GetEdit(_connections.GetConnections(Context.ConnectionId));
        _editMapping.Remove(_connections.GetConnections(Context.ConnectionId));
        await OpenEdit(id);
        await Clients.Others.SendAsync("Changed", client.Id, client.Title);
    }
