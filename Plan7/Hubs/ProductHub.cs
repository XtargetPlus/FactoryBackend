...
public class ProductHub : Hub
{
    ...
    public ProductHub(IProductService productRepository, IProductCountService count)
    {
    ...
    {
        ProductGetDto? product = await _productRepository.GetFirstAsync(addedId);
        if (_productRepository.HasErrors || product == null)
            return;
        await Clients.Others.SendAsync("Added", product.ProductId, product.Price, product.DetailId, await _count.GetAllAsync());
    }
    ...
    {
        ProductGetDto? product = await _productRepository.GetFirstAsync(changedId);
        if (_productRepository.HasErrors || product == null)
            return;
        int id = _editMapping.GetEdit(_connections.GetConnections(Context.ConnectionId));
        _editMapping.Remove(_connections.GetConnections(Context.ConnectionId));
        await OpenEdit(id);
        await Clients.Others.SendAsync("Changed", product.ProductId, product.Price, product.DetailId);
    }
