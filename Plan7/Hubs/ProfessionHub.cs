...
/// </summary>
[Authorize]
public class ProfessionHub : Hub
{
    ...
    public ProfessionHub(IProfessionService professionRepository, IProfessionCountService count)
    {
    ...
    {
        BaseDto? profession = await _professionRepository.GetFirstAsync(addedId);
        if (_professionRepository.HasErrors || profession == null)
            return;
        await Clients.Others.SendAsync("Added", profession.Id, profession.Title, await _count.GetAllAsync());
    }
    ...
    {
        BaseDto? profession = await _professionRepository.GetFirstAsync(changedId);
        if (_professionRepository.HasErrors || profession == null)
            return;
        int id = _editMapping.GetEdit(_connections.GetConnections(Context.ConnectionId));
        _editMapping.Remove(_connections.GetConnections(Context.ConnectionId));
        await OpenEdit(id);
        await Clients.Others.SendAsync("Changed", profession.Id, profession.Title);
    }
