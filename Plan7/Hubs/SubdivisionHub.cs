...
/// </summary>
[Authorize]
public class SubdivisionHub : Hub
{
    private readonly ISubdivisionService _subdivisionRepository;
    private readonly static ConnectionMapping _connections = new();
    private readonly static EditMapping _editMapping = new();

    public SubdivisionHub(ISubdivisionService subdivisionRepository)
    {
    ...
    {
        SubdivisionGetDto? subdiv = await _subdivisionRepository.GetFirstAsync(addedId);
        if (_subdivisionRepository.HasErrors || subdiv == null)
            return;
        await Clients.Others.SendAsync("Added", subdiv.Id, subdiv.Title, subdiv.FatherId, subdiv.CountChildren);
    }
    ...
    {
        SubdivisionGetDto? subdiv = await _subdivisionRepository.GetFirstAsync(changedId);
        if (_subdivisionRepository.HasErrors || subdiv == null)
            return;
        int id = _editMapping.GetEdit(_connections.GetConnections(Context.ConnectionId));
        _editMapping.Remove(_connections.GetConnections(Context.ConnectionId));
        await OpenEdit(id);
        await Clients.Others.SendAsync("Changed", subdiv.Id, subdiv.Title, subdiv.FatherId, subdiv.CountChildren);
    }
