...
public class MoveTypeHub : Hub
{
    ...
    public MoveTypeHub(ISimpleGenericModelService<MoveType> service)
    {
        _service = service;
    }
    ...
    /// </summary>
    /// <returns>Список изменяемых записей в виде List<int></returns>
    [Authorize]
    ...
    {
        BaseTitleModel? moveType = await _service.GetFirstAsync(addedId);
        if (_service.HasErrors || moveType == null)
            return;
        await Clients.Others.SendAsync("Added", moveType.Id, moveType.Title);
    }
    ...
    {
        BaseTitleModel? moveType = await _service.GetFirstAsync(changedId);
        if (_service.HasErrors || moveType == null)
            return;
        int id = _editMapping.GetEdit(_connections.GetConnections(Context.ConnectionId));
        _editMapping.Remove(_connections.GetConnections(Context.ConnectionId));
        await OpenEdit(id);
        await Clients.Others.SendAsync("Changed", moveType.Id, moveType.Title);
    }
