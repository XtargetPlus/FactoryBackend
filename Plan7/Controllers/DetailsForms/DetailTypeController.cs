...
/// <summary>
/// Контроллер типа детали
/// </summary>
public class DetailTypeController : SimpleController<BaseDto, DetailTypeController>
{
    private readonly ISimpleGenericModelService<DetailType> _service;
    ...
    public DetailTypeController(ILogger<DetailTypeController> logger, ISimpleGenericModelService<DetailType> service)
    ...
    [HttpPost]
    [Authorize(Roles = DetailTypeControllerRoles.Add)]
    public override async Task<IActionResult> Add(TitleDto dto)
    {
        _logger.LogInformation("Пользователь {userId}: Добавление типа детали: {Title}", HttpContext.User.FindFirstValue("UserId"), dto.Title);
    ...
    [HttpPost]
    [Authorize(Roles = DetailTypeControllerRoles.Change)]
    public override async Task<IActionResult> Change(BaseDto dto)
    {
        _logger.LogInformation("Пользователь {userId}: Изменение типа детали: {DetailTypeId} - {Title}", HttpContext.User.FindFirstValue("UserId"), dto.Id, dto.Title);
    ...
    [HttpDelete("{id:int}")]
    [Authorize(Roles = DetailTypeControllerRoles.Delete)]
    public override async Task<IActionResult> Delete(int id)
    {
        _logger.LogInformation("Пользователь {userId}: Удаление типа детали: {DetailTypeId}", HttpContext.User.FindFirstValue("UserId"), id);
    ...
    [HttpGet]
    [Authorize(Roles = DetailTypeControllerRoles.GetAll)]
    public override async Task<IActionResult> GetAll([FromQuery, MaybeNull] string text)
    {
        _logger.LogInformation("Пользователь {userId}: Получение списка типов деталей", HttpContext.User.FindFirstValue("UserId"));
